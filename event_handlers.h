...
void* user_alive(gpointer user) {
    ...
    while(rows > 0) {
        ...
        // def - callstats.h
        gint8 rc = callstats_user_alive(user, janus_get_real_time());
        if (rc == TRUE) {
        ...
        // def - data_store.h
        rows = get_user_info(user.session_id,
                             user.handle_id,
                             &user);
    }
...
void plugin_eventhandler(json_t *event) {
    ...
    if (strcmp(event_name, "joined") == 0) {
        ...
        GError *error = NULL;
        GThread *thread = g_thread_try_new(NULL, (GThreadFunc*)user_alive, (gpointer)user, &error);
        if (error == NULL) {
